...
//        release_image (&this->m_Mask);
    ...
    Calibration::load_camera_parameters ()
    {
        ...
        for (int i = 0; i < this->m_NumCams; i++)
        {
            ...
            sprintf (str, "calibration.cameras.camera_%d.disto_1", i);
            if (config_get_double (this->m_ConfigHandle, str, &disto_1) < 0)
            {
              printf ("Error: Cannot load disto_1 for camera_%d\n", i);
              return -1;
            }
            sprintf (str, "calibration.cameras.camera_%d.disto_2", i);
            if (config_get_double (this->m_ConfigHandle, str, &disto_2) < 0)
            {
              printf ("Error: Cannot load disto_2 for camera_%d\n", i);
              return -1;
            }
            sprintf (str, "calibration.cameras.camera_%d.disto_3", i);
            if (config_get_double (this->m_ConfigHandle, str, &disto_3) < 0)
            {
                printf ("Error: Cannot load disto_3 for camera_%d\n", i);
                return -1;
            }
            sprintf (str, "calibration.cameras.camera_%d.disto_4", i);
            if (config_get_double (this->m_ConfigHandle, str, &disto_4) < 0)
            {
                printf ("Error: Cannot load disto_4 for camera_%d\n", i);
                return -1;
            }
            sprintf (str, "calibration.cameras.camera_%d.disto_5", i);
            if (config_get_double (this->m_ConfigHandle, str, &disto_5) < 0)
            {
                printf ("Error: Cannot load disto_5 for camera_%d\n", i);
                return -1;
            }
            sprintf (str, "calibration.cameras.camera_%d.disto_6", i);
            if (config_get_double (this->m_ConfigHandle, str, &disto_6) < 0)
            {
                printf ("Error: Cannot load disto_6 for camera_%d\n", i);
                return -1;
            }

            //generate K matrix
            this->m_Calib.K[i][0] = focal_length;
            this->m_Calib.K[i][1] = 0;
            this->m_Calib.K[i][2] = camera_center_X;
            this->m_Calib.K[i][3] = 0;
            this->m_Calib.K[i][4] = focal_length;
            this->m_Calib.K[i][5] = camera_center_Y;
            this->m_Calib.K[i][6] = 0;
            this->m_Calib.K[i][7] = 0;
            this->m_Calib.K[i][8] = 1;

            //generate distortion matrix
            this->m_Calib.disto[i][0] = disto_1;
            this->m_Calib.disto[i][1] = disto_2;
            this->m_Calib.disto[i][2] = disto_3;
            this->m_Calib.disto[i][3] = disto_4;
            this->m_Calib.disto[i][4] = disto_5;
            this->m_Calib.disto[i][5] = disto_6;
...
//    Calibration::load_point_cloud (std::string filename)
    ...
    Calibration::load_image (int imageIndex)
    {
        ...
        char *image_type;
        image_type = config_get_str_or_default (this->m_ConfigHandle, "calibration.cameras.image_type", (char*)"jpg");
    ...
    /**
     * This function project the camera coordinate to image coordinate
     */
     void
     Calibration::project_cam2img(const double *x, const double *k, const double *d, int *x_)
     {
         double x1 = x[0]/x[2];
         double y1 = x[1]/x[2];
         double r_2 = x1*x1 + y1*y1;
         double x2 = (1+k[0]*r_2+k[1]*r_2*r_2+k[2]*r_2*r_2)/(1+k[3]*r_2+k[4]*r_2*r_2+k[5]*r_2*r_2*r_2)*x1;
         double y2 = (1+k[0]*r_2+k[1]*r_2*r_2+k[2]*r_2*r_2)/(1+k[3]*r_2+k[4]*r_2*r_2+k[5]*r_2*r_2*r_2)*y1;
         int u = k[0]*x2 + k[2];
         int v = k[4]*y2 + k[5];
         x_[0] = u;
         x_[1] = v;
                ...
                for (int j = 0; j < this->m_NumCams; j++)
                {
                    ...
                    if (point.range < 1)
                    {
                        ...
                        if (camera_xyz[2] > 0)
                        {
                            ...
                            int image_point[2];
                            project_cam2img(camera_xyz, this->m_Calib.K[j], this->m_Calib.disto[j], image_point);
